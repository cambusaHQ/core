        ...
        try {
          switch (relationOptions.type) {
            ...
            case 'one-to-one': {
              ...
              if (
                typeof relatedData !== 'number' &&
                typeof relatedData !== 'string'
              ) {
                throw new Error(
              ...
              const relatedEntity = await cambusa.models[
                relationOptions.target
              ].findOne({
            ...
            default:
              throw new Error(
                `Unsupported relation type: '${relationOptions.type}' for relation '${relationName}'.`
              );
          }
        } catch (error) {
          throw new Error(
            `Error processing relation '${relationName}': ${error.message}`
          );
        }
      }
    }
