    ...
    if (!disabledRoutes.includes('create')) {
      router.post('', async ({ body, error }) => {
        try {
          ...
          cambusa.log.error(`Error creating ${model.options.name}:`, err.message);
          return error(400, `Error creating ${model.options.name}: ${err.message}`);
        }
      });
    }
      ...
      router.get('', async ({ query, set, error }) => {
        try {
          ...
          if (invalidPopulate.length > 0) {
            return error(400, `Invalid relations to populate: ${invalidPopulate.join(', ')}`);
          }
          ...
          cambusa.log.error(`Error fetching ${model.options.name}s:`, err.message);
          return error(400, `Error fetching ${model.options.name}s: ${err.message}`);
        }
      });
    }
      ...
      router.get('/:id', async ({ params, query, error }) => {
        try {
          ...
          if (result) {
            ...
            return error(404, `${model.options.name} not found`);
          }
        } catch (err) {
          cambusa.log.error(`Error fetching ${model.options.name}:`, err.message);
          return error(400, `Error fetching ${model.options.name}: ${err.message}`);
        }
      });
    }
      ...
      router.put('/:id', async ({ params, body, error }) => {
        try {
          ...
          if (updated) {
            ...
            return error(404, `${model.options.name} not found after update.`);
          }
        } catch (err) {
          cambusa.log.error(`Error updating ${model.options.name}:`, err.message);
          return error(400, `Error updating ${model.options.name}: ${err.message}`);
        }
      });
    }
    ...
    if (!disabledRoutes.includes('delete')) {
      router.delete('/:id', async ({ params, error }) => {
        try {
          ...
          cambusa.log.error(`Error deleting ${model.options.name}:`, err.message);
          return error(400, `Error deleting ${model.options.name}: ${err.message}`);
        }
      });
    }
