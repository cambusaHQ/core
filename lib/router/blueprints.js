...
/**
 * Refactored blueprints module to support relations, pagination, filtering, and header-based meta.
 * @param {Array} shema - Array of model definitions.
 * @returns {Array} - Array of Elysia routers.
 */
export function blueprints(shema) {
  const routers = [];
  ...
  shema.forEach((model) => {
     ...
     * Supports filtering via 'where' query parameter (JSON).
     * Uses 'populate' instead of 'relations' for including related data.
     * Returns data directly and exposes meta in headers.
     */
    if (!disabledRoutes.includes('readAll')) {
      router.get('', async ({ query, set }) => {
        try {
          ...
          // Fetch entities with relations, filtering, and pagination
