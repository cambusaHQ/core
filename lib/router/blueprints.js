      ...
      router.post(
        ...
        async ({ body, error }) => {
          try {
            ...
            return error(400, {
              error: `Error creating ${schema.name}: ${err.message}`,
            });
          }
        },
        {
          ...
          try {
            ...
            if (query.where) {
              try {
                ...
                return error(400, {
                  error: `'where' parameter must be valid JSON.`,
                });
              }
            }
            ...
            return error(400, {
              error: `Error fetching ${schema.name}s: ${err.message}`,
            });
          }
        },
        {
      ...
      router.get(
        ...
        async ({ params, query, error }) => {
          try {
            ...
            return error(400, {
              error: `Error fetching ${schema.name}: ${err.message}`,
            });
          }
        },
        {
        ...
        async ({ params, body, error }) => {
          try {
            ...
            if (updated) {
              ...
              return error(404, {
                error: `${schema.name} not found after update.`,
              });
            }
          } catch (err) {
            cambusa.log.error(`Error updating ${schema.name}:`, err.message);
            return error(400, {
              error: `Error updating ${schema.name}: ${err.message}`,
            });
          }
        },
        {
    ...
    if (!disabledRoutes.includes('delete')) {
      router.delete(
        ...
        async ({ params, error }) => {
          ...
          try {
            entity = await cambusa.models[schema.name].findOneBy({
              id,
            });
            if (!entity) throw new Error();
          } catch (err) {
            cambusa.log.debug(`Error deleting ${schema.name}: ${err.message}`);
            return error(404, {
              error: `${schema.name} ${id} not found.`,
            });
          }

          try {
            await cambusa.models[schema.name].remove(entity);

            return { message: `${schema.name} ${id} deleted.` };
          } catch (err) {
            cambusa.log.error(`Error deleting ${schema.name}: ${err.message}`);
            return error(500, {
              error: `Error deleting ${schema.name}: ${err.message}`,
            });
          }
        },
        {
